    ...
    stages {
            ...
            steps {
                script {                // Groovy Script for Building Docker Image
                    withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker24') {
                       ...
                       dockerImage = sh 'docker buildx build --platform linux/amd64,linux/arm64,linux/arm/v7 -t ${IMAGE_NAME}:${IMAGE_TAG} .'
                    }
                }
            }
        }
        ...
        stage ('Push to Docker Image Registry') {
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker24') {
                        sh 'docker push --platform linux/amd64,linux/arm64,linux/arm/v7 -t ${IMAGE_NAME}:${IMAGE_TAG}'
                    //    dockerImage.push()
