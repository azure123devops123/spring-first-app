  ...
  stages {
    ...
    stage ('Analyze Image to Find CVEs') {
      steps {
        ...
        script {
					docker.withRegistry('','DockerhubID') {   // Login into Dockerhub
              sh 'docker-scout cves $IMAGE_TAG --exit-code --only-severity critical'    // Only Failure if the vulnerabilities are CRITICAL
          }
        }
      }
    }
